...
function activate(context) {
    ...
    const chatParticipant = vscode.chat.createChatParticipant('serenitydev.assistant', async (request, context, response, token) => {
    ...
    const lmProvider = {
        async provideLanguageModelChatInformation(options, token) {
            try {
                const response = await axios_1.default.get(`${API_BASE}/models`);
    ...
    context.subscriptions.push(vscode.lm.registerLanguageModelChatProvider('serenitydev', lmProvider));
    let isPolling = false;
    async function updateStatusBar() {
        ...
        isPolling = true;
        try {
            const response = await axios_1.default.get(`${API_BASE}/status`);
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(SerenityChatProvider.viewType, provider));
...
class SerenityChatProvider {
    _extensionUri;
    static viewType = 'serenity.chatView';
    _view;
    _sessionId;
    _generateSessionId() {
        return `session_${(0, crypto_1.randomBytes)(16).toString('hex')}`;
    }
            ...
            if (data.type === 'sendQuery') {
                try {
                    const statusInterval = setInterval(async () => {
                        try {
                            const statusRes = await axios_1.default.get(`${API_BASE}/status`);
                    ...
                    const response = await axios_1.default.post(ASK_URL, {
                        prompt: data.value,
                ...
                try {
                    await axios_1.default.delete(`${API_BASE}/session/${this._sessionId}`);
                }
                catch (e) { }
                this._sessionId = this._generateSessionId();
            }
