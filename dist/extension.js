...
var require_form_data = __commonJS({
  "node_modules/form-data/lib/form_data.js"(exports2, module2) {
    ...
    FormData3.prototype._multiPartHeader = function(field, value, options) {
      ...
      var escapeHeaderParam = function(param) {
        if (typeof param !== "string") {
          return param;
        }
        return param.replace(/\r/g, "%0D").replace(/\n/g, "%0A").replace(/"/g, "%22");
      };
      var contents = "";
      var headers = {
    ...
    FormData3.prototype._getContentDisposition = function(value, options) {
      ...
      if (filename) {
        filename = filename.replace(/\r/g, "%0D").replace(/\n/g, "%0A").replace(/"/g, "%22");
...
function setProxy(options, configProxy, location, isRedirect, configHttpsAgent) {
  ...
  if (proxy && typeof proxy === "object" && !(proxy instanceof URL)) {
    if (!Object.prototype.hasOwnProperty.call(configProxy, "host") && !Object.prototype.hasOwnProperty.call(configProxy, "hostname") && !Object.prototype.hasOwnProperty.call(configProxy, "protocol")) {
      proxy = void 0;
    }
  }
...
function mergeConfig(config1, config2) {
  ...
  const mergeMap = {
    ...
    proxy: mergeDirectKeys,
    auth: mergeDirectKeys,
    headers: (a, b, prop) => mergeDeepProperties(headersToObject(a), headersToObject(b), prop, true)
  };
  const allKeys = Array.from(/* @__PURE__ */ new Set([...Object.keys(config1 || {}), ...Object.keys(config2 || {})]));
  utils_default.forEach(allKeys, function computeConfigValue(prop) {
...
function dispatchRequest(config) {
  ...
  if (config && typeof config === "object") {
    const cleanConfig = /* @__PURE__ */ Object.create(null);
    ...
    if (!synchronousRequestInterceptors) {
      const normalizeConfig = (cfg) => {
        if (cfg && typeof cfg === "object") {
          return mergeConfig(this.defaults, cfg);
        }
        return cfg;
      };
      const chain = [function(cfg) {
        return dispatchRequest.call(this, normalizeConfig(cfg));
      }.bind(this), void 0];
...
var serverProcess;
var serverOutputChannel;
var API_BASE = "http://localhost:8002/api";
var ASK_URL = "http://localhost:8002/ask";
function activate(context) {
  ...
  const chatParticipant = vscode.chat.createChatParticipant("serenitydev.assistant", async (request2, context2, response, token) => {
    response.progress("Initializing SerenityDev routing pipeline...");
    return new Promise((resolve, reject) => {
      let fullPrompt = request2.prompt;
      if (request2.command) {
        fullPrompt = `/${request2.command} ${fullPrompt}`;
      }
...
> \u{1F5FA}\uFE0F **Routing:** \`Supervisor\` \u27A1\uFE0F \`Worker: ${routing.worker}\`
> \u{1F50D} **Review:** \`${routing.review_badge}\``;
                  if (routing.steps && routing.steps.length > 0) {
                    routingInfo += `
>
> \u{1F6E0}\uFE0F **Agentic Tools Executed:**
`;
                    routing.steps.forEach((s) => {
                      const icon = s.status === "success" ? "\u{1F7E2}" : "\u{1F7E1}";
...
`;
  ...
  function getModelCapabilities(modelName) {
    ...
    const imageInput = nameLower.includes("-vl") || nameLower.includes("vision") || nameLower.includes("llava") || nameLower.includes("paligemma") || nameLower.includes("minicpm-v");
    const isBaseOrFim = nameLower.includes("fim") || nameLower.includes("-2b") || nameLower.includes("base");
  ...
  const lmProvider = {
    async provideLanguageModelChatInformation(options, token) {
      try {
        const response = await axios_default.get(`${API_BASE}/models`);
...
`;
  ...
  async function updateStatusBar() {
    ...
    isPolling = true;
    try {
      const response = await axios_default.get(`${API_BASE}/status`);
  ...
  const interval = setInterval(updateStatusBar, 5e3);
...
var SerenityChatProvider = class {
  ...
  _sessionId;
  _generateSessionId() {
    return `session_${(0, import_crypto2.randomBytes)(16).toString("hex")}`;
  }
      ...
      if (data.type === "sendQuery") {
        try {
          const statusInterval = setInterval(async () => {
            try {
              const statusRes = await axios_default.get(`${API_BASE}/status`);
          ...
          const response = await axios_default.post(ASK_URL, {
            prompt: data.value,
            ...
            <script>
                ...
                window.addEventListener('message', event => {
                    ...
                    if (message.type === 'updateStatus') {
                        ...
                        let routingHtml = '';
                        if (message.routing) {
                            routingHtml = '<div class="routing-box">\u{1F5FA}\uFE0F Plan Step Count: ' + (message.routing.steps ? message.routing.steps.length : 0) + '<br/>Worker: ' + message.routing.worker + '</div>';
                        }
