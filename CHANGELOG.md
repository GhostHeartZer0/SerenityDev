...
- **Documentation & Model Architecture Chart Overhaul**:
  ...
  - Added dedicated License & Credits attribution block (Author GhostHeartZer0, llama.cpp, MCP, model providers, and backend frameworks); verified Apache 2.0 `LICENSE.md` compliance.

- **MCP Architecture Hardening & Code Improvements**:
  - **JSON-RPC Spec Alignment**: Updated `/mcp` route in `serenitydevserver.py` to advertise `tools`, `resources`, and `prompts` capabilities with `listChanged` flags. Added `"initialized"` method handshake alias alongside `"notifications/initialized"`. Standardized `tools/call` parsing to handle arguments wrapped in `arguments`, `parameters`, or stringified JSON with automatic deserialization. Guaranteed `-32601` method not found errors include incoming request `id`.
  - **Dynamic MCP Resource Registration**: Added `resources/list` and `resources/read` handlers in `serenitydevserver.py`, exposing `workspace://files` (JSON file listing) and `workspace://git_status` (working tree diff/status) with directory exclusion filters (`.git`, `.venv`, `node_modules`, `__pycache__`, `.env`).
  - **Native MCP TLS & Multi-Interface SAN Binding**: Enhanced `start_native_mcp.py` to detect all local network adapter IP addresses via `get_all_local_ips()` and bind them along with system hostname into the leaf certificate `SubjectAlternativeName`. Subclassed `ReusableHTTPServer` (`allow_reuse_address = True`) and served `rootCA.pem` / `rootCA.crt` with `application/x-x509-ca-cert` for immediate Android certificate installation. Ignored `*.crt` and `rootCA.crt` in `.gitignore`.
  - **VS Code Client Auth Token Synchronization**: Updated `serenityHttp` in `src/extension.ts` to automatically attach `Authorization: Bearer <token>` when `process.env.SERENITY_MCP_TOKEN` or configured `serenitydev.mcpToken` is present. Contributed `serenitydev.mcpToken` setting in `package.json`.
  - **Automated Verification**: Expanded unit test suite `src/test/test_mcp.py` covering initialize capabilities, notification aliases, parameter variations, dynamic resource listing and reading, error conditions, and network address detection. All 29 unit tests pass.
